...
# load minimum data -----------------------------------------
data<-read.csv("../data/maxtemp.csv")
names(data)
...
pval$pvalue #pvalue = 0.1365 - moderate evidence
...
wpval<-westfall_young_pvalue(argvals=argvals, resamples_mean=res$resamples_mean, sample_mean=res$sample_mean, include_group=include_group, split=argvalsindex)
wpval$raw_pvalue
wpval$corr_pvalue
...
int_plot<-ggplot()+geom_line(data=datlong, aes(x=as.Date(argvals, origin="2011-01-01 00:00:00"), y=value, group=variable), colour=gray, alpha=0.2)+
  ...
  labs(title="",x ="Time", y = "Max Temperature Interaction", col="")+timescale+mytheme+
...
main_plot1<-ggplot()+geom_line(data=datlong, aes(x=as.Date(argvals, origin="2011-01-01 00:00:00"), y=value, group=variable), colour=gray, alpha=0.2)+
  ...
  labs(title="",x ="Time", y = bquote(paste("Max Temperature Difference (",tilde(H)," - N", tilde(H), ")", sep="")), col="")+timescale+mytheme+
  annotate(geom="text", x=as.Date("2011-09-27"), y=Inf,label=paste("global p-value=",pval$pvalue), hjust = 1, vjust = 1)
...
main_plot2<-ggplot()+geom_line(data=datlong, aes(x=as.Date(argvals, origin="2011-01-01 00:00:00"), y=value, group=variable), colour=gray, alpha=0.2)+
  ...
  #scale_color_manual(labels = c("Resamples",bquote(paste("Sample(",tilde(H),"S)"))), values=c('black', gray))+
  theme(legend.title = element_blank())+
  labs(title="",x ="Time", y = bquote(paste("Max Temperature Difference (",tilde(S)," - N", tilde(S), ")", sep="")), col="")+timescale+mytheme+
  annotate(geom="text", x=as.Date("2011-09-27"), y=Inf,label=paste("global p-value=",pval$pvalue),hjust = 1, vjust = 1)
...
d1<-which(wpval$corr_pvalue<=0.05)
seg1<-data.frame(time=as.Date(argvals[argvalsindex[[d1]]],origin="2011-01-01 00:00:00"),sig_diff=dat$dataframe$sample[argvalsindex[[d1]]])
d2<-which(wpval$corr_pvalue<=0.10 & wpval$corr_pvalue>0.05)
seg2<-data.frame(time=as.Date(argvals[argvalsindex[[d2[1]]]],origin="2011-01-01 00:00:00"),sig_diff=dat$dataframe$sample[argvalsindex[[d2[1]]]])
seg3<-data.frame(time=as.Date(argvals[argvalsindex[[d2[2]]]],origin="2011-01-01 00:00:00"),sig_diff=dat$dataframe$sample[argvalsindex[[d2[2]]]])
seg4<-data.frame(time=as.Date(argvals[argvalsindex[[d2[3]]]],origin="2011-01-01 00:00:00"),sig_diff=dat$dataframe$sample[argvalsindex[[d2[3]]]])

corrected_main_plot2<-main_plot2+
  geom_line(data=seg1, aes(x=time, y=sig_diff), colour="red", alpha=1, size=1)+
  geom_line(data=seg2, aes(x=time, y=sig_diff), colour="yellow", alpha=1, size=1)+
  geom_line(data=seg3, aes(x=time, y=sig_diff), colour="yellow", alpha=1, size=1)+
  geom_line(data=seg4, aes(x=time, y=sig_diff), colour="yellow", alpha=1, size=1)
